/**
 * @see https://github.com/hairyf/unstorage/blob/main/test/drivers/utils.ts
 */
...
export function testDriver(opts: TestOptions): void {
    ...
    it("init", async () => {
        await restoreSnapshot(ctx.storage, {
            initial: "works",
        });
    ...
    it("serialize (object)", async () => {
        await ctx.storage.setItem("/data/test.json", {
            json: "works",
        });
    ...
    it("serialize (lossy object with toJSON())", async () => {
        ...
        expect(
            await ctx.storage.getItem("/data/serialized2.json"),
        ).toMatchObject({
            serializedObj: "works",
        });
    });
